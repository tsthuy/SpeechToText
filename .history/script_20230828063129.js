...
//       saveText(document.querySelector(".interim").innerHTML); // Lưu văn bản đã chuyển đổi
//     };
...
function speechToText() {
  try {
    recognition = new SpeechRecognition();
    recognition.lang = inputLanguage.value;
    recognition.interimResults = false; // Đặt interimResults thành false
    recordBtn.classList.add("recording");
    recordBtn.querySelector("p").innerHTML = "Listening...";
    recognition.start();
    recognition.onresult = (event) => {
      const speechResult = event.results[0][0].transcript;
      result.innerHTML = speechResult; // Hiển thị kết quả cuối cùng

      // Lưu văn bản đã chuyển đổi
...
function clearSavedText() {
  ...
  savedTextsElement.innerHTML = 'Chưa có văn bản nào được lưu.';
}
blank.addEventListener('click', clearSavedText);
